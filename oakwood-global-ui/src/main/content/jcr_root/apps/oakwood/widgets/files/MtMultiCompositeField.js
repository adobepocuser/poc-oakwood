...
//    },
    ...
    reorder: function(item) {
		...
		this.fields.each(function(f) {
            f.name=f.name.replace(name,itemName);
            if(f.fileNameParameter){
				f.fileNameParameter  = f.fileNameParameter.replace(name,itemName);
            }

            if(f.fileReferenceParameter){
				f.fileReferenceParameter = f.fileReferenceParameter.replace(name,itemName);
            }

        });

        item.fields.each(function(f) {
            f.name=f.name.replace(itemName,name);
             if(f.fileNameParameter){
				f.fileNameParameter  = f.fileNameParameter.replace(itemName,name);
            }

            if(f.fileReferenceParameter){
				f.fileReferenceParameter = f.fileReferenceParameter.replace(itemName,name);
            }
        });
    ...
    processRecord: function(record,path) {
        this.fields.each(function(f) {
            if (f.processRecord) {
            	//just trigger that event;
            	f.fireEvent('beforeloadcontent', this, record, path)
            }
        });
    },
